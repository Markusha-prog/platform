...
class CloseTeam(View, LoginRequiredMixin):
    ...
    def get(self, request):
        a = Team.objects.get(pk=request.user.profile.team_user.id)
        a.user_teams.remove(request.user)
        if a.capitan == request.user:
            a.capitan = None
            a.save()
        user_model = Profile.objects.get(user=request.user)
        user_model.team_user = None
        user_model.save()
